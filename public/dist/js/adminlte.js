...
(function (global, factory) {
  ...
  function llenarEmpresaCA() {
    $.post('usuario/empresaCA', function (e) {
      $('#empresaCA').html(e);
      llenarsucursalCA();
    });
  }

  window.llenarsucursalCA = function() {
    var cod =  $('#txtempresaCA').val();
    $.post('usuario/sucursalCA',{'id':cod}, function (e) {
      $('#sucursalCA').html(e);
      llenaralmacenCA()
    });
  }

  window.llenaralmacenCA = function() {
    var cod =  $('#txtsucursalCA').val();
    $.post('usuario/almacenCA',{'id':cod}, function (e) {
      $('#almacenCA').html(e);
    });
  }

  $("#frmcambioalmacenAC").on('submit', function (e) {
    e.preventDefault();
    var idal = document.getElementById('txtalmacenCA');            
    var nombre_almacen =idal.options[idal.selectedIndex].text;

    var idsu = document.getElementById('txtsucursalCA');            
    var nombre_sucursal =idsu.options[idsu.selectedIndex].text;

    var datos = new FormData();
    datos.append('idsucursal', $('#txtsucursalCA').val());
    datos.append('idalmacen', $('#txtalmacenCA').val());
    datos.append('nombre_almacen', nombre_almacen);
    datos.append('nombre_sucursal', nombre_sucursal);
    $.ajax({
        type: 'POST',
        url: 'usuario/CambioAlmacen/',
        data: datos,
        contentType: false,
        cache: false,
        processData: false,
        success: function (msg) {
            ...
            console.log(data);
            if (data.includes("LISTO")) { 
              $('#almacenModalCenter').modal('hide');               
                Swal.fire({
                    icon: 'success',
                    title: data,
                }).then(function () {                    
  ...
  function responsiveDesign() {
    var anchoPantalla = $(window).innerWidth(),
       ...
       // console.log(anchoPantalla,altoPantalla);
        if (anchoPantalla < 360 || anchoPantalla == 360) {
            $('.title-rx').removeClass('h6');
        } else {
            $('.title-rx').addClass('h6');
        }   
}


$(window).on('resize',responsiveDesign);
