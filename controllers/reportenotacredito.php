<?php
class Reportenotacredito extends Controller
{
    function __construct()
    {
        ...
        $this->view->js = array("reportenotacredito/js/script-reportenotacredito.js");
    }

    public function index()
    {
        if (Session::get('rutas')[22]['estado'] == '1') {
            $this->view->render('reportenotacredito/index');# code...
        } else {
    ...
    function rutaCdr()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/cdr_nc/'.$ruta;
    ...
    function rutaXml()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/xml_firmar_nc/'.$ruta;
            header ("Content-Disposition: attachment; filename=".$ruta." ");
            header ("Content-Type: application/octet-stream");
            header ("Content-Length: ".filesize($enlace));
            readfile($enlace);
