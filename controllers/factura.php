...
class Factura extends Controller {
    ...
    function __construct()
    {
        ...
        $this->view->js = array("factura/js/script-factura.js");
    }
    
    public function index()
    {
        if (Session::get('rutas')[16]['estado'] == '1') {
            $this->view->render('factura/index');# code...
        } else {
    ...
    function listarVentaPeriodo() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            foreach ($variable as $key => $value) {
                ...
                $boton = "<div class='d-flex'><button type='button' data-ventaid = ".$value['codigo']." class='ver btn btn-xs'><kbd class='bg-danger'>pdf</kbd></button><button type='button' data-xml = 'sin_ruta' class='xml btn btn-xs'><kbd class='bg-primary'>xml</kbd></button><button type='button' data-cdr ='sin_ruta' class='cdr btn btn-xs'><kbd class='bg-warning'>cdr</kbd></button></div>";
    ...
    function rutaCdr()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/cdr_factura/'.$ruta;
    ...
    function rutaXml()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/xml_firmar_factura/'.$ruta;
            header ("Content-Disposition: attachment; filename=".$ruta." ");
            header ("Content-Type: application/octet-stream");
            header ("Content-Length: ".filesize($enlace));
            readfile($enlace);
