...
class Notacredito extends Controller
{
    function __construct()
    {
        ...
        $this->view->js = array("notacredito/js/script-notacredito.js");
    }

    public function index()
    {
        if (Session::get('rutas')[18]['estado'] == '1') {
            $this->view->render('notacredito/index');# code...
        } else {
    ...
    function odtenerSerieCorrelativo()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $mensaje = $this->model->obtenerSerieCorre($almacen,$tipocomp,$referencia);
    ...
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $this->view->Listar = $this->model->listarMotivo('C');
    ...
    function buscarComprobante() 
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $mensaje = $this->model->listarComprobante($serie,$correlativo);
            echo json_encode($mensaje);
        }
    }
    ...
    function ListarDetalleVenta() 
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $codigo = $_POST['codigo'];
            $detalle = $this->model->listarDetalleVenta($codigo);
            
            if (!isset($_SESSION['carrito_nc'])) {
                $_SESSION['carrito_nc'] = array();
            }else{
                unset($_SESSION['carrito_nc']);
                $_SESSION['carrito_nc'] = array();
            }
            ...
            foreach ($detalle as $key => $value) {
                $canasta = array(
                    'codigo' => $value['codigo'],
                    'idexistencia_lotes' => $value['idexistencia_lotes'],
                    'sku' => $value['sku'],
            ...
            $_SESSION['carrito_nc'] = $this->view->carrito;
            ...
            $this->view->render('notacredito/tabla/tablaVenta', true);
        }
    }
    ...
    function ItemDellCarrito() 
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $this->view->carrito = $_SESSION['carrito_nc'];
            $existe = false;
            $item = 0;
            foreach ($this->view->carrito as $k => $v) {
    ...
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $this->view->render('notacredito/tabla/tablaVentaNew', true);
        }
    }

    function insertarNotaCredito(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if(isset($_SESSION['carrito_nc']) && Session::get('idCaja') !== '0'){
                $aux = array();
                $aux = $_SESSION['carrito_nc'];
                ...
                if(count($aux)>0){
                    ...
                    $mensaje = $this->model->insertNotaCredito($content);
                    unset($_SESSION['carrito_nc']);
                    echo json_encode($mensaje);
                }else{
    ...
    public function sendToSunat(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
           ...
           if($encabezado['idtipocomp'] == '07'){
                    ...
                    $motivo =  $this->model->obtenerMotivo('C',$encabezado['motivo_baja']);
                    $see = new See();
                   ...
                   // ->setCodLocal($empresa['CODIGO_ESTABLECIMIENTO']); // Codigo de establecimiento asignado por SUNAT, 0000 por defecto.
                    ...
                    $note
                        ...
                        if($encabezado['formapago'] !=='Contado') {                            
                            $note->setFormaPago(new FormaPagoCredito($encabezado['total']))
                                    ->setCuotas([
                                        (new Cuota())
                       ...
                       // Guardar XML firmado digitalmente.                       
                        file_put_contents('./facturacion/xml_firmar_nc/'.$note->getName().'.xml',$see->getFactory()->getLastXml());
                        ...
                        // Guardamos el CDR
                        file_put_contents('./facturacion/cdr_nc/'.'R-'.$note->getName().'.zip', $result->getCdrZip());     

                        $zip = new ZipArchive;
                            // Declaramos el fichero a descomprimir, puede ser enviada desde un formulario
                        $comprimido= $zip->open('./facturacion/cdr_nc/'.'R-'.$note->getName().'.zip');
                        if ($comprimido === TRUE) {                    
                            // Declaramos la carpeta que almacenara ficheros descomprimidos
                            $zip->extractTo('./facturacion/cdr_nc/');
                            $zip->close();
                            rmdir('./facturacion/cdr_nc/dummy');
                        }                   
