...
class Boleta extends Controller
{
    function __construct()
    {
        ...
        $this->view->js = array("boleta/js/script-boleta.js");
    }
    
    public function index()
    {
        if (Session::get('rutas')[17]['estado'] == '1') {
            $this->view->render('boleta/index');# code...
        } else {
    ...
    function listarVentaPeriodo() 
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            foreach ($variable as $key => $value) {
                ...
                if ($value['feestado'] == '109' || $value['feestado'] == '') {
                    $condicion = "<kbd class='bg-warning' >No Enviado</kbd>";
                    $boton = "<div class='d-flex'><button type='button' data-sunat= ".$value['codigo']." class='sunat btn btn-xs'><kbd class='bg-success'>sunat</kbd></button><button type='button' data-ventaid = ".$value['codigo']." class='ver btn btn-xs'><kbd class='bg-danger'>pdf</kbd></button></div>";
                }
    ...
    function rutaCdr()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/cdr_boleta/'.$ruta;
    ...
    function rutaXml()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/xml_firmar_boleta/'.$ruta;
            header ("Content-Disposition: attachment; filename=".$ruta." ");
            header ("Content-Type: application/octet-stream");
            header ("Content-Length: ".filesize($enlace));
            readfile($enlace);
