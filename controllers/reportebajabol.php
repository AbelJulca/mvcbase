<?php
class Reportebajabol extends Controller {
    ...
    function __construct()
    {
        ...
        $this->view->js = array("reportebajabol/js/script-reportebajabol.js");
    }
    
    public function index()
    {
        if (Session::get('rutas')[25]['estado'] == '1') {
            $this->view->render('reportebajabol/index');# code...
        } else {
    ...
    function rutaCdr()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/cdr_rc/'.$ruta;
    ...
    function rutaXml()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $enlace = './facturacion/xml_firmar_rc/'.$ruta;
            header ("Content-Disposition: attachment; filename=".$ruta." ");
            header ("Content-Type: application/octet-stream");
            header ("Content-Length: ".filesize($enlace));
            readfile($enlace);
